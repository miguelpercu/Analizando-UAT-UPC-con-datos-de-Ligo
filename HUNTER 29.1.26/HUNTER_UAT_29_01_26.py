...
class PulseHunter:
    ...
    def apply_percudani_filter(self, data, epsilon=1e-12):
        """ Filtro con protección contra colapso matemático (nan) """
        ...
        # Normalización blindada: evita la división por cero
    ...
    def analyze_real_data(self, h1_path, l1_path):
        print(f"\n" + "="*60)
        print(f"🚀 RESONANT HUNTER v3.9.2 - EJECUCIÓN DATOS REALES")
        print(f"ESTADO: PROTECCIÓN DE FASE ACTIVADA | NODO 14")
        print("="*60)
        ...
        try:
            ...
            print(f"✓ Carga exitosa: {len(h1_raw)} muestras analizadas.")
            ...
            print(f"COH MÁXIMA: {max_coh:.8f}")
            print(f"SNR CUÁNTICO: {snr_q:.8f}")
            print(f"ESTADO: {'🔥 ANOMALÍA CONFIRMADA' if snr_q >= 1.5 else 'VIGILANCIA'}")
            print(f"-"*44)

            if snr_q >= 1.5:
                peak_f = freqs[higo_mask][np.argmax(coh_val[higo_mask])]
                print(f"🎯 Resonancia detectada en: {peak_f:.4f} Hz")

        except Exception as e:
            print(f"❌ Error crítico en el Nodo 14: {e}")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    input("[NODO 14] Análisis completado. Presiona ENTER para ver el reporte final...")
...
class CausalScanner:
    def __init__(self):
        ...
        # Parámetros del Scanner (Ecuación 4 - Localización)
        self.segment_duration = 4  # Ventanas de 4 segundos
        self.step_size = 1        # Desplazamiento de 1 segundo (alta resolución)
    ...
    def run_scan(self, h1_path, l1_path):
        print(f"🚀 INICIANDO SCANNER CAUSAL UAT v3.9.3")
        print(f"Buscando anomalías segundo a segundo...")
        ...
        for start in range(0, total_samples - samples_per_segment, samples_step):
            ...
            if snr_q > 0.05: # Umbral de pre-alerta
                ...
                print(f"[{t_offset:7.1f}s] COH: {max_coh:.4f} | SNR: {snr_q:.4f} | F: {peak_f:.2f} Hz {'🔥' if snr_q >= 1.5 else ''}")
                if snr_q >= 1.5: found_anomalies += 1

        print(f"\nScan finalizado. Anomalías críticas encontradas: {found_anomalies}")
